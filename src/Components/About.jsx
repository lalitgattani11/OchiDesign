      ...
      <div className="flex flex-col lg:flex-row border-t border-[#6e7f31] pt-10 mt-16 gap-8">
        ...
        <div className="w-full lg:w-1/2">
          ...
          <Link
            ...
            className="flex items-center gap-3 px-6 py-4 sm:px-8 sm:py-5 uppercase bg-zinc-800 mt-6 text-white rounded-full w-fit hover:bg-black hover:font-semibold transition-colors duration-500 ease-in-out"
          >
            Read More
            <div className="flex justify-center items-center relative w-8 h-8 flex-shrink-0">
              <div
                className={`rounded-full transition-all duration-500 ease-in-out bg-zinc-100 flex items-center justify-center 
                  ${isHovered ? "w-8 h-8" : "w-2.5 h-2.5"}`}
              >
                {isHovered && <GoArrowUpRight className="text-black text-lg" />}
              </div>
            </div>
          </Link>
        </div>
